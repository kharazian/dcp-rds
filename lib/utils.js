 ...
 * @param {string}              elementId       A unique identifier for the resource within the job (eg slice number)
 */
exports.storeContent = async function utils$$storeContent(content, contentType, job, elementType, elementId)
{
  job         = encodeURI(job);
  elementType = encodeURI(elementType);
  elementId   = encodeURI(elementId);
  
  const dir = path.join(jobsDir, job, elementType, elementId);
 ...
 * @param {string} elementId       A unique identifier for the resource within the job (eg slice number)
 */
exports.sendContent = function utils$$sendContent(response, job, elementType, elementId)
{
  try
  {
    job         = encodeURI(job);
    ...
    const filenameStub = path.join('jobs',  job, elementType, elementId, 'content');
    const filename     = path.join(jobsDir, job, elementType, elementId, 'content-type');
    if (!filename.startsWith(jobsDir))
      throw new Error('Invalid pathname');
